
import ansible_runner
import os
import shutil

from modules.simulation_controller import SimulationController
from modules import aws_service, azure_service


class ArtSimulationController(SimulationController):

    def simulate(self, target, technique) -> None:
        if self.config["general"]["cloud_provider"] == "aws":
            # Get both public and private IP, prefer public if available
            instance = aws_service.get_instance_details(
                target,
                self.config["general"]["key_name"],
                self.config["general"]["attack_range_name"],
                self.config["aws"]["region"],
            )
            if instance:
                try:
                    public_ip = instance['NetworkInterfaces'][0]['Association'].get('PublicIp', None)
                except Exception:
                    public_ip = None
                private_ip = instance['NetworkInterfaces'][0]['PrivateIpAddress']
                ip_address = public_ip if public_ip else private_ip
            else:
                raise Exception(f"Could not find instance with name {target}")
            private_key_path = self.config["aws"]["private_key_path"]
            if "win" in target:
                ansible_user = "Administrator"
                ansible_port = 5985
                cmd_line = str("-i " + ip_address + ", ")
            else:
                ansible_user = "ubuntu"
                ansible_port = 22
                cmd_line = (
                    "-u "
                    + ansible_user
                    + " --private-key "
                    + private_key_path
                    + " -i "
                    + ip_address
                    + ", "
                )

        elif self.config["general"]["cloud_provider"] == "azure":
            target_instance = azure_service.get_instance(
                target,
                self.config["general"]["key_name"],
                self.config["general"]["attack_range_name"],
            )
            public_ip = target_instance.get("public_ip")
            private_ip = target_instance.get("private_ip")
            ip_address = public_ip if public_ip else private_ip
            if "win" in target:
                ansible_user = "AzureAdmin"
                ansible_port = 5985
                cmd_line = str("-i " + ip_address + ", ")
            else:
                ansible_user = "ubuntu"
                ansible_port = 22
                cmd_line = (
                    "-u "
                    + ansible_user
                    + " --private-key "
                    + self.config["azure"]["private_key_path"]
                    + " -i "
                    + ip_address
                    + ", "
                )
            private_key_path = self.config["azure"]["private_key_path"]

        elif self.config["general"]["cloud_provider"] == "gcp":
            instance = gcp_service.get_instance_details(
                target,
                self.config["general"]["key_name"],
                self.config["gcp"]["region"],
            )
            if instance:
                public_ip = instance.get("public_ip")
                private_ip = instance.get("private_ip")
                ip_address = public_ip if public_ip else private_ip
            else:
                raise Exception(f"Could not find instance with name {target}")
            private_key_path = self.config["gcp"]["private_key_path"]
            if "win" in target:
                ansible_user = "Administrator"
                ansible_port = 5985
                cmd_line = str("-i " + ip_address + ", ")
            else:
                ansible_user = "ubuntu"
                ansible_port = 22
                cmd_line = (
                    "-u "
                    + ansible_user
                    + " --private-key "
                    + private_key_path
                    + " -i "
                    + ip_address
                    + ", "
                )

        techniques = technique.split(",")

        if "win" in target:
            runner = ansible_runner.run(
                private_data_dir=os.path.join(os.path.dirname(__file__), "../"),
                cmdline=cmd_line,
                roles_path=os.path.join(os.path.dirname(__file__), "ansible/roles"),
                playbook=os.path.join(
                    os.path.dirname(__file__), "ansible/atomic_red_team.yml"
                ),
                extravars={
                    "ansible_port": ansible_port,
                    "ansible_connection": "winrm",
                    "ansible_winrm_server_cert_validation": "ignore",
                    "techniques": techniques,
                    "ansible_user": ansible_user,
                    "ansible_password": self.config["general"]["attack_range_password"],
                    "art_repository": self.config["simulation"]["atomic_red_team_repo"],
                    "art_branch": self.config["simulation"]["atomic_red_team_branch"],
                    "cap_attack": self.config["simulation"]["cap_attack"],
                    "cap_attack_upload_threat_capture": self.config["simulation"][
                        "cap_attack_upload_threat_capture"
                    ],
                },
                verbosity=0,
            )

        elif "linux" in target:
            runner = ansible_runner.run(
                private_data_dir=os.path.join(os.path.dirname(__file__), "../"),
                cmdline=cmd_line,
                roles_path=os.path.join(os.path.dirname(__file__), "ansible/roles"),
                playbook=os.path.join(
                    os.path.dirname(__file__), "ansible/atomic_red_team.yml"
                ),
                extravars={
                    "ansible_port": ansible_port,
                    "ansible_python_interpreter": "/usr/bin/python3",
                    "techniques": techniques,
                    "art_repository": self.config["simulation"]["atomic_red_team_repo"],
                    "art_branch": self.config["simulation"]["atomic_red_team_branch"],
                    "cap_attack": self.config["simulation"]["cap_attack"],
                    "cap_attack_upload_threat_capture": self.config["simulation"][
                        "cap_attack_upload_threat_capture"
                    ],
                },
                verbosity=0,
            )

        shutil.rmtree(os.path.join(os.path.dirname(__file__), "../artifacts"))
        shutil.rmtree(os.path.join(os.path.dirname(__file__), "../env"))
